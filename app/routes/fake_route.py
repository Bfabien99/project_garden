...
from models import Organization, RetrieveOrganisation, Project, RetrieveProject, ProjectStatus
...
async def create_fake_organization(session: SessionDep, uuid: str):
    organization = session.exec(select(Organization).where(Organization.uuid == uuid)).one_or_none()
    if not organization:
        raise HTTPException(
    ...
    project = Project(
        title=title, description=description, logo=logo, start_date=start_date, end_date=end_date, organization_id=organization.uuid, status=status
    )
    session.add(project)
    session.commit()
    session.refresh(project)
    return RetrieveProject(
        uuid=project.uuid, title=project.title, description=project.description, logo=project.logo, start_date=project.start_date, end_date=project.end_date, organization_id=organization.uuid, status=project.status
    )