...
async def add_an_project(project: CreateProject, session: SessionDep):
    ...
    organization = session.exec(select(Organization).where(Organization.uuid == project.organization_id)).one_or_none()
    if not organization:
        raise HTTPException(
    ...
    save_pro = Project(
        title=title, description=description, logo=logo, start_date=project.start_date, end_date=project.end_date, organization_id=project.organization_id, status=project.status
    )
    session.add(save_pro)
    session.commit()
    session.refresh(save_pro)
    return RetrieveProject(
        uuid=save_pro.uuid, title=save_pro.title, description=save_pro.description, logo=save_pro.logo, start_date=save_pro.start_date, end_date=save_pro.end_date, organization_id=organization.uuid, status=save_pro.status
    )
...
async def get_a_project(
    ...
    org = session.exec(select(Organization).where(Organization.uuid == project.organization_id)).one_or_none()
    return {"project": project, "organization": org}

@router.put("/{uuid}", response_model=RetrieveProject)
async def update_a_project(
    session: SessionDep, uuid: str, update_pro: UpdateProject
):
    project = get_by_uuid(uuid, session)
    if not project:
        raise HTTPException(
    ...
    if update_pro.organization_id:
        organization = session.exec(select(Organization).where(Organization.uuid == update_pro.organization_id)).one_or_none()
        if not organization:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Organisation #{project.organization_id} not found",
    ...
    return RetrieveProject(
        uuid=project.uuid, title=project.title, description=project.description, logo=project.logo, start_date=project.start_date, end_date=project.end_date, organization_id=project.organization_id, status=project.status
    )
